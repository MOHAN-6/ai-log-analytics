...
# ============================================
# GENERATE ENTERPRISE LOGS - FIXED!
# ============================================
def generate_enterprise_logs(count=10000):
    ...
    # FIXED: Each pattern has the correct number of placeholders
    log_patterns = {
        'INFO': [
    ...
    for i in range(count):
        ...
        # FIXED: Count placeholders and fill accordingly
        try:
            if pattern.count('{}') == 2:
                message = pattern.format(service, random.choice([instance_id, bucket_name, user, str(random.randint(100, 9999))]))
            elif pattern.count('{}') == 3:
                message = pattern.format(service, random.choice([instance_id, bucket_name]), random.randint(10, 999))
            else:
                message = pattern.format(service)
        except:
            message = f"{service} - {pattern.replace('{}', str(random.randint(1, 999)))}"
...
def bulk_ingest():
    ...
    return jsonify({"success": True, "ingested": ingested, "total_logs": storage.total_logs})
